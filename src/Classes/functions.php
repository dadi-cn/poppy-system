...
use Poppy\System\Classes\Cacher;
...
if (!function_exists('sys_seo')) {
	function sys_seo(...$args)
	{
		...
		View::share([
			'_title'       => $title ?: sys_setting('system::site.title'),
			'_description' => $description ?: sys_setting('system::site.description'),
		]);
	}
}
...
if (!function_exists('sys_parent_id')) {
	...
	function sys_parent_id($pam)
	{
		...
		if (!$rel) {
			$rel = sys_cache('system')->get('system.pam_parent_rel');
		}
		...
		if (!isset($rel[$pamId])) {
			...
			sys_cache('system')->forever('system.pam_parent_rel', $rel);
		}
...
if (!function_exists('sys_mix')) {
	 ...
	 * @return mixed
	 */
	function sys_mix($key)
	{
		return route('system:web.res.mix', [$key]);
	}
}
...
if (!function_exists('sys_content_trim')) {
	 ...
	 * @return string
	 */
	function sys_content_trim($content, $allowable_tags = 'p,img')
	{
