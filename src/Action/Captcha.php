...
class Captcha
{
    ...
    {
        if (!$captcha) {
            return $this->setError(trans('py-system::action.image_captcha.check_not_input'));
        }
        ...
        if (!$captchaCache) {
            return $this->setError(trans('py-system::action.image_captcha.check_not_exist'));
        }

        if (strtolower($captchaCache) !== strtolower($captcha)) {
            return $this->setError(trans('py-system::action.image_captcha.check_error'));
        }
     ...
     */
    public function generate($key, $length = 4)
    {
